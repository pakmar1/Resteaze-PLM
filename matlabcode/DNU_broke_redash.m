...
output.fileName='Richard Allen'; %%%%%%%%%%%%%%%%%%%%%%%%%Temp name for report eventually should use some characters from filename
%
[leftFileNames,leftPath]=uigetfile('*.csv','LEFT FILES','multiselect','on'); %%Should have date in beginning of filename so they are sorted from old->new
...
for i=1:max([size(leftFileNames,2) size(rightFileNames,2)])
    ...
    output.rRMS=rms(removeAccGrav(rightLeg(:,8:10)),2);
    output.lRMS=rms(removeAccGrav(leftLeg(:,8:10)),2);
